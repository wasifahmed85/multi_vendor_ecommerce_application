...
function getSubAreas(operationAreaId, route, operationSubAreaId = null) {

    axios.get(route, {
        params: { operationArea_Id: operationAreaId }
    })
        .then(function (response) {
            if (response.data.operation_sub_areas.length > 0) {
                $('#operation_sub_area').html(`<option value="" selected hidden>Select Operation Sub Area</option>`);
                ...
                $('#operation_sub_area').html(`<option value="" selected hidden>Select Operation Sub Area</option>`).prop('disabled', true);
            }
        })
        .catch(function (error) {
            console.error(error);
            $('#operation_sub_area').html(`<option value="" selected hidden>Select Operation Sub Area</option>`).prop('disabled', true);
            alert('Failed to load operation Sub areas.');
        });
}
